...
const Page = () => {
  ...
  if (isNaN(d.getTime())) {
    ...
    return ""; // or return original value
  }
    ...
    const fetchData = async () => {
      try {
        ...
        if (responseData.success && Array.isArray(responseData.data)) {
          ...
          const transformed: PublicationItem[] = items.map(
            (item: any, index: number) => {
              const plain = item.toObject ? item.toObject() : item;
              const base = {
                ...plain,
                id: (index + 1).toString(), // display ID
                _id: plain._id ? plain._id.toString() : undefined, // db ID used for operations
              };
              if (item.FilingDate)
               base.FilingDate = formatDate(item.FilingDate);
              if (item.GrantDate)
               base.GrantDate = formatDate(item.GrantDate);
              return base;
            },
          );
  ...
  const getTitle = (): string => {
    switch (pageType) {
      case "patent":
        return "Patents (Admin)";
      case "journal":
        return "Journal Publications (Admin)";
      case "conference":
        return "Conference Proceedings (Admin)";
      case "bookchapter":
        return "Book Chapters (Admin)";
      default:
        return "Publications (Admin)";
    }
  };
  ...
  const getItemYear = (item: PublicationItem): string | null => {
    ...
    if ("FilingDate" in item)
      return extractYearMap((item as Patent).FilingDate);
                  ...
                  {availableYears.map((year) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center w-full text-left px-4 py-2 text-sm hover:bg-blue-100 ${isSelected ? "bg-blue-50 text-blue-700 font-semibold" : "text-gray-700"}`}
                      >
                        <input
                          type="checkbox"
                          checked={isSelected}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6">
            {filteredData.map((item) => (
              <div
                ...
                className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 p-6 border border-blue-100 hover:border-blue-300 flex flex-col h-full group"
              >
            ...
            <div className="p-6 overflow-y-auto">
              <form
                onSubmit={async (e) => {
                  ...
                  setIsSaving(true);
                  const apiType =
                    pageType === "bookchapter" ? "journal" : pageType;
                  ...
                  try {
                    ...
                    // alert('this is item to edit: ' + itemToEdit._id);
                    // const numItemToEdit= Number(itemToEdit._id)
                    // alert(`Updating ${apiType} with ID ${numItemToEdit}:\n` + JSON.stringify(dataObj, null, 2));
              ...
              >
                ...
                {pageType === "patent" && (
                  <>
                    <InputField
                      ...
                      name="Applno"
                      defaultValue={(itemToEdit as Patent).Applno}
                    />
                    <InputField
                      label="Status"
                      name="Status"
                      defaultValue={(itemToEdit as Patent).Status}
                    />
                    <InputField
                      label="Inventors"
                      name="Inventors"
                      defaultValue={(itemToEdit as Patent).Inventors}
                    />
                    <div className="flex gap-4">
                      <div className="flex-1">
                        <InputField
                          ...
                          type="date"
                          name="FilingDate"
                          defaultValue={(itemToEdit as Patent).FilingDate}
                        />
                      </div>
                      <div className="flex-1">
                        <InputField
                          ...
                          type="date"
                          name="GrantDate"
                          defaultValue={(itemToEdit as Patent).GrantDate}
                        />
                      </div>
                    </div>
                  </>
                ...
                {(pageType === "journal" || pageType === "bookchapter") && (
                  <>
                    <InputField
                      label="Authors"
                      name="authors"
                      defaultValue={(itemToEdit as Journal).authors}
                    />
                    <InputField
                      label="Journal"
                      name="journal"
                      defaultValue={(itemToEdit as Journal).journal}
                    />
                    <div className="flex gap-4">
                      <div className="flex-1">
                        <InputField
                          ...
                          type="number"
                          name="year"
                          defaultValue={(itemToEdit as Journal).year}
                        />
                      </div>
                      <div className="flex-1">
                        <InputField
                          label="Volume"
                          name="volume"
                          defaultValue={(itemToEdit as Journal).volume}
                        />
                      </div>
                    </div>
                    <div className="flex gap-4">
                      <div className="flex-1">
                        <InputField
                          label="Issue"
                          name="issue"
                          defaultValue={(itemToEdit as Journal).issue}
                        />
                      </div>
                      <div className="flex-1">
                        <InputField
                          label="Pages"
                          name="pages"
                          defaultValue={(itemToEdit as Journal).pages}
                        />
                      </div>
                    </div>
                    <div className="flex gap-4">
                      <div className="flex-1">
                        <InputField
                          label="Type"
                          name="type"
                          defaultValue={(itemToEdit as Journal).type}
                        />
                      </div>
                      <div className="flex-1">
                        <InputField
                          label="Status"
                          name="status"
                          defaultValue={(itemToEdit as Journal).status}
                        />
                      </div>
                    </div>
                  </>
                ...
                {pageType === "conference" && (
                  <>
                    <InputField
                      label="Authors"
                      name="authors"
                      defaultValue={(itemToEdit as Conference).authors}
                    />
                    <InputField
                      label="Conference"
                      name="conference"
                      defaultValue={(itemToEdit as Conference).conference}
                    />
                    <div className="flex gap-4">
                      <div className="flex-1">
                        <InputField
                          ...
                          type="number"
                          name="year"
                          defaultValue={(itemToEdit as Conference).year}
                        />
                      </div>
                      <div className="flex-1">
                        <InputField
                          label="Month"
                          name="month"
                          defaultValue={(itemToEdit as Conference).month}
                        />
                      </div>
                    </div>
                    <InputField
                      label="Location"
                      name="location"
                      defaultValue={(itemToEdit as Conference).location}
                    />
                    <div className="flex gap-4">
                      <div className="flex-1">
                        <InputField
                          label="Pages"
                          name="pages"
                          defaultValue={(itemToEdit as Conference).pages}
                        />
                      </div>
                      <div className="flex-1">
                        <InputField
                          label="Type"
                          name="type"
                          defaultValue={(itemToEdit as Conference).type}
                        />
                      </div>
                    </div>
                    <InputField
                      label="Status"
                      name="status"
                      defaultValue={(itemToEdit as Conference).status}
                    />
                  </>
                )}
                ...
                <div className="mt-6 flex justify-end gap-3 pt-4 border-t border-gray-200">
                  <button
                    type="button"
                    onClick={closeEditModal}
                    className="px-5 py-2.5 text-gray-900 bg-gray-200 hover:bg-gray-300 font-medium rounded-lg transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={isSaving}
                    className="px-5 py-2.5 text-white bg-blue-600 hover:bg-blue-700 font-medium rounded-lg transition-colors"
                  >
